...
- [ ] 1.1 In `cmd_close` (src/cli.rs), change `was_active` to be computed from the resolved
      session id only: `store::read_active_session().await.as_deref() == Some(&session_id)`
      (drop the `session_arg.is_none()` condition).
- [ ] 1.2 Verify existing behavior preserved: `close` with no arg on the active session still
...
- [ ] 2.1 `test_session_close_alias_clears_active` — `use <sess>` then `session close <sess>`;
      assert `use` (no arg) reports no active session and `list` shows no `*` on the closed row.
- [ ] 2.2 `test_close_explicit_id_clears_active` — `use <sess>` then `close <sess>`; assert the
      same clean state.
- [ ] 2.3 `test_close_non_active_keeps_active_marker` — two sessions, active = A; `close B`;
      assert active still A and `*` remains on A.
- [ ] 2.4 Existing tests unchanged and passing (test_close_session, test_close_json_output,
...
- [ ] 3.4 Live verification on shared daemon (repro from cycle-08 feedback)
