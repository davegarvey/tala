...
- [ ] 1.1 Add `seen: Option<String>` (URL-encoded JSON map session_id→last-seen)
      to `WaitNewParams`; parse into `HashMap<String, u64>`
- [ ] 1.2 `wait_new_session`: pass parsed `seen` into `find_incoming_session`
- [ ] 1.3 `find_incoming_session(store, me, seen)`: candidate = open session
...
- [ ] 4.3 Full `cargo test` passes (incl. #46 suite)
- [ ] 4.4 Live repro on shared daemon: stale session not delivered; fresh
