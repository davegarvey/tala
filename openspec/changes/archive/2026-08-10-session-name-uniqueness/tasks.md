...
- [x] 1.1 Add `pub async fn session_name_exists(&self, name: &str) -> bool` —
      any session in `self.sessions` with `name == Some(name)`.
- [x] 1.2 `rename_session`: before mutating, scan sessions; if a session with
...
- [x] 2.1 `create_session` handler: if `req.name` is Some and
...
- [x] 2.2 `rename_session` handler: already maps `Err(msg)` → 409; no change
      needed beyond the store change.

## 3. CLI (src/cli.rs)

- [x] 3.1 `auto_create_session`: after `send().await?`, inspect `resp.status()`;
...
- [x] 4.4 `test_rename_duplicate_json_error`: `--json` variant asserts
      `"code":"SESSION_NAME_TAKEN"`.
- [x] 4.5 Regression: `test_send_without_active_session_auto_creates` stays
      green (auto-create with no name unaffected).

## 5. Quality gates

- [x] 5.1 `cargo fmt --check` clean
- [x] 5.2 `cargo clippy -- -D warnings` clean
- [x] 5.3 `cargo test` green (full suite)
- [x] 5.4 Live-verify on shared daemon: dup create + dup rename + self-rename
      + `--json` paths
