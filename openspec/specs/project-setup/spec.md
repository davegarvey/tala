...
`tala init` SHALL install the tala opencode skill and command files into the project's `.opencode/` directory when one exists, so the project's agent knows how to use tala.
...
- **THEN** a command file SHALL be created at `.opencode/commands/tala.md`
