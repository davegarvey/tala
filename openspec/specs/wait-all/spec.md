...
`tala wait` without `--session` or active session, when 2+ open sessions exist on the daemon, SHALL call `/api/sessions/wait-all` and display the first received message.

#### Scenario: No explicit session, multiple open sessions
- **WHEN** `tala wait` runs with 2+ open sessions and no `--session` or active session set
